...
module.exports.add = function(req, res) {
    async.parallel([
        ...
        var array = [];
        var stream = models.Roles2.find().sort({"roleID":1}).stream();
            ...
            function updateAclAlerts(aclAlertToUpdate1, typeAclAlert){
                models[typeAclAlert].find({}, function(err, groups) {
                    if( err || !groups) console.log("No Alerts groups found");
                        ...
                        if (group.checkBoxID == 's'+aclAlertToUpdate1+group.alertID){
                        ...
                        if (group.checkBoxID == 'r'+aclAlertToUpdate1+group.alertID){
                            group.roleGroupID = req.body.roleID;
                            group.roleGroupName = req.body.roleName;
